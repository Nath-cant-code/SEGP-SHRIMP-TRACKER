 ...
 *
 * Backend contract (from shared/api-contract.md):
 *   GET  /models                          → { models: [{id, label}] }
 *   POST /analyze (multipart/form-data)   → AnalyzeResponse
 *   GET  /results                         → { results: [ResultListItem] }
 *   GET  /results/{job_id}/{video_id}/csv → CSV download
 */
...
// ─── Theme tokens (applied via CSS vars) ─────────────────────────────────────
const DARK = {
  "--bg":          "#090d16",
  "--surface":     "#0f1623",
  "--card":        "#131c2e",
  ...
  "--accent":      "#14b8a6",
  "--accent-dim":  "#0d9488",
  "--accent-faint":"rgba(20,184,166,0.10)",
  "--amber":       "#f59e0b",
  "--amber-faint": "rgba(245,158,11,0.09)",
  "--coral":       "#f87171",
  "--coral-faint": "rgba(248,113,113,0.09)",
  "--blue":        "#60a5fa",
  "--blue-faint":  "rgba(96,165,250,0.09)",
  "--success":     "#34d399",
  "--warn":        "#fbbf24",
  "--err":         "#f87171",
  "--scrollbar":   "#1a2740",
...
const LIGHT = {
  ...
  "--accent":      "#0d9488",
  "--accent-dim":  "#0f766e",
  "--accent-faint":"rgba(13,148,136,0.08)",
  "--amber":       "#b45309",
  "--amber-faint": "rgba(180,83,9,0.07)",
  "--coral":       "#dc2626",
  "--coral-faint": "rgba(220,38,38,0.07)",
  "--blue":        "#2563eb",
  "--blue-faint":  "rgba(37,99,235,0.07)",
  "--success":     "#059669",
  "--warn":        "#d97706",
  "--err":         "#dc2626",
  "--scrollbar":   "#c5d5e8",
...
function uploadWithProgress(formData, onProgress, signal) {
  return new Promise((resolve, reject) => {
    ...
    xhr.timeout = 300_000; // 5 min for large videos
...
const SERIES_COLORS = [
  { line: "#14b8a6", dash: false },  // teal   - video 1
  { line: "#f59e0b", dash: true  },  // amber  - video 2
  { line: "#f87171", dash: true  },  // coral  - video 3
];
...
function LineChart({ datasets, field, unit, smoothing }) {
  ...
  if (!allVals.length || !allTimes.length) {
    return <div style={{ height: H, display: "flex", alignItems: "center", justifyContent: "center", color: "var(--text-muted)", fontSize: 13 }}>No data</div>;
  }
  ...
  const buildPath = (ds) =>
    ds.timeseries.map((p, i) => {
      ...
      return `${i === 0 || ds.vals.slice(0, i).every(x => x == null) ? "M" : "L"}${tx(p.time_sec).toFixed(1)},${ty(v).toFixed(1)}`;
    }).filter(Boolean).join(" ");
  ...
  const onMouseMove = useCallback((e) => {
    ...
    const hoverData = processed.map(ds => {
      ...
      return { label: ds.video_name, val: ds.vals[idx], time: ds.timeseries[idx].time_sec, x: tx(ds.timeseries[idx].time_sec), y: ty(ds.vals[idx] ?? 0), color: ds.color.line };
    });
    setHover(hoverData);
    ...
    <div style={{ position: "relative", width: "100%" }}>
      <svg ref={svgRef} viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", display: "block", cursor: "crosshair" }}
        ...
        {gridVals.map((v, i) => (
          <g key={i}>
            ...
            <text x={PAD.l - 6} y={ty(v) + 4} textAnchor="end" fill={textFill} fontSize="10" fontFamily="'SF Mono',monospace">
              {v.toFixed(0)}
            </text>
          </g>
        ))}
        {gridTimes.map((t, i) => (
          <text key={i} x={tx(t)} y={H - PAD.b + 13} textAnchor="middle" fill={textFill} fontSize="10" fontFamily="'SF Mono',monospace">
            {t.toFixed(1)}s
          </text>
        ))}
        <text x={-H / 2} y={15} transform="rotate(-90)" textAnchor="middle" fill={textFill} fontSize="10" fontFamily="'SF Mono',monospace">
          {unit}
        </text>
        ...
        {processed.map((ds, di) => {
          ...
          return (
            <g key={di}>
              <path
                d={`${path} L${tx(lastPt.time_sec)},${PAD.t + IH} L${tx(firstPt.time_sec)},${PAD.t + IH} Z`}
                fill={`url(#grad-${field}-${di})`}
              />
              <path
                d={path} fill="none" stroke={ds.color.line} strokeWidth="1.8"
        ...
        {hover && (
          <>
            <line x1={hover[0].x} y1={PAD.t} x2={hover[0].x} y2={PAD.t + IH} stroke={textFill} strokeWidth="0.8" strokeDasharray="3 2" />
            {hover.map((h, i) => h.val != null && (
              <circle key={i} cx={h.x} cy={h.y} r="4" fill={h.color} stroke={dark ? "#090d16" : "#ffffff"} strokeWidth="1.5" />
            ))}
          </>
        )}
      </svg>
      ...
      {hover && (
        <div style={{
          ...
          pointerEvents: "none", fontFamily: "'SF Mono',monospace", color: "var(--text-sec)",
          minWidth: 170, boxShadow: "0 4px 16px rgba(0,0,0,0.2)",
        }}>
          <div style={{ color: "var(--text-muted)", marginBottom: 3, fontSize: 10 }}>t = {hover[0]?.time.toFixed(2)}s</div>
          {hover.map((h, i) => h.val != null && (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <span style={{ width: 8, height: 8, borderRadius: 2, background: h.color, display: "inline-block", flexShrink: 0, outline: SERIES_COLORS[i % SERIES_COLORS.length].dash ? `1.5px dashed ${h.color}` : "none", outlineOffset: 1 }} />
              <span style={{ color: "var(--text-pri)", fontWeight: 500 }}>{h.val.toFixed(2)}</span>
              <span style={{ color: "var(--text-muted)", fontSize: 10 }}>{unit}</span>
              <span style={{ color: "var(--text-muted)", fontSize: 10, marginLeft: "auto", maxWidth: 80, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{shortName(h.label, 14)}</span>
            </div>
          ))}
        </div>
      )}
...
function MetricCard({ label, value, unit, accent }) {
  return (
    ...
    >
      ...
      <div style={{ fontSize: 21, fontWeight: 600, color: accent || "var(--text-pri)", letterSpacing: "-0.02em", fontFamily: "'SF Mono',monospace", lineHeight: 1 }}>
        {value}
...
function AlertLog({ entries }) {
  ...
  return (
    <div ref={ref} style={{ height: 180, overflowY: "auto", fontFamily: "'SF Mono',monospace", fontSize: 11.5, lineHeight: 1.9 }}>
      {entries.length === 0 && (
...
function UploadZone({ files, onFiles }) {
  ...
  const validate = (fileList) => {
    ...
    if (valid.length + files.length > MAX_VIDEOS) {
      alert(`Maximum ${MAX_VIDEOS} videos allowed per analysis.`);
      return valid.slice(0, MAX_VIDEOS - files.length);
    }
    return valid;
  };
        ...
        <div style={{ marginTop: 8, display: "flex", flexDirection: "column", gap: 5 }}>
          {files.map((f, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, background: "var(--surface)", borderRadius: 6, padding: "6px 10px", border: "1px solid var(--border)", fontSize: 12 }}>
              <span style={{ color: "var(--accent)", fontFamily: "'SF Mono',monospace", fontSize: 10 }}>V{i + 1}</span>
  ...
  return (
    <div style={{ marginTop: 10 }}>
      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "var(--text-muted)", marginBottom: 5 }}>
        <span>{label}</span>
        <span style={{ fontFamily: "'SF Mono',monospace" }}>{value}%</span>
      </div>
...
// ─── Summary Cards for a video result ────────────────────────────────────────
...
// ─── Past Jobs Table ──────────────────────────────────────────────────────────
function PastJobsTable({ jobs, onReload }) {
  ...
  return (
    <div style={{ overflowX: "auto" }}>
      <table style={{ width: "100%", borderCollapse: "collapse" }}>
        <thead>
          <tr>
            {["Job ID", "Created", "Model", "Videos"].map(h => <th key={h} style={hdSty}>{h}</th>)}
          </tr>
        </thead>
        <tbody>
          {jobs.map(j => (
            <tr key={j.job_id} style={{ transition: "background 0.1s" }}
              onMouseEnter={e => e.currentTarget.style.background = "var(--surface)"}
              onMouseLeave={e => e.currentTarget.style.background = "transparent"}
            >
              <td style={{ ...cellSty, fontFamily: "'SF Mono',monospace", color: "var(--accent)", fontSize: 11 }}>{j.job_id}</td>
...
function StatusBadge({ status }) {
  ...
  return (
    <span style={{ background: bg, color: c, fontSize: 9.5, fontWeight: 700, letterSpacing: "0.1em", padding: "3px 10px", borderRadius: 4, fontFamily: "'SF Mono',monospace", display: "inline-flex", alignItems: "center", gap: 5 }}>
...
export default function ShrimpDashboard() {
  ...
  const [analyzePct, setAnalyzePct] = useState(0); // simulated inference progress
  const [status, setStatus] = useState("idle"); // idle | uploading | analyzing | done | error
  const [result, setResult] = useState(null);   // AnalyzeResponse from backend
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        log(`Loaded ${data.models.length} YOLO model(s) from server`, "ok", 200);
      } catch (e) {
  ...
  const fetchPastJobs = useCallback(async () => {
    ...
    try {
      ...
      log(`Could not load past jobs: ${e.message}`, "warn");
    } finally {
    ...
    try {
      ...
      const data = await uploadWithProgress(fd, (pct) => {
        ...
        if (pct === 100) {
          ...
          setStatus("analyzing");
          // Simulate inference progress (backend is synchronous; we animate until response arrives)
          let ap = 0;
    ...
    <ThemeContext.Provider value={{ dark, toggle: () => setDark(d => !d) }}>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        :root { font-size: 14px; }
        body { background: var(--bg); color: var(--text-pri); font-family: 'Sora', system-ui, sans-serif; line-height: 1.6; -webkit-font-smoothing: antialiased; }
      ...
      <div style={{ minHeight: "100vh", background: "var(--bg)", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ display: "flex", flex: 1, minHeight: 0 }}>
          ...
          <aside style={{ width: 268, flexShrink: 0, background: "var(--surface)", borderRight: "1px solid var(--border)", display: "flex", flexDirection: "column", overflowY: "auto", padding: "20px 16px" }}>
            ...
            <SideSection label="Advanced">
              <button onClick={() => setShowAdvanced(a => !a)} style={{ width: "100%", background: "none", border: "1px solid var(--border)", borderRadius: 6, color: "var(--text-sec)", fontSize: 12, padding: "7px 0", cursor: "pointer", transition: "border-color 0.15s", fontFamily: "inherit" }}
                onMouseEnter={e => e.currentTarget.style.borderColor = "var(--accent)"}
                onMouseLeave={e => e.currentTarget.style.borderColor = "var(--border)"}
              >
              ...
              {showAdvanced && (
                <div style={{ marginTop: 10, fontSize: 11.5, color: "var(--text-muted)", lineHeight: 1.8 }}>
                  <p>Additional model parameters (confidence threshold, max match distance) are configured in the backend <code style={{ background: "var(--border)", padding: "1px 4px", borderRadius: 3 }}>.env</code> file. Reload this page after changing them.</p>
                  <p style={{ marginTop: 8 }}>Backend URL: <code style={{ background: "var(--border)", padding: "1px 4px", borderRadius: 3 }}>{BASE_URL}</code></p>
                </div>
              )}
            </SideSection>
                  ...
                  <>
                      ...
                      <div style={{ display: "flex", gap: 16, fontSize: 12, color: "var(--text-sec)" }}>
                        {legend.map((l, i) => (
                          <span key={i} style={{ display: "flex", alignItems: "center", gap: 6 }}>
                            <span style={{ width: 22, height: 3, background: l.color, borderRadius: 2, display: "inline-block", backgroundImage: l.dash ? `repeating-linear-gradient(90deg,${l.color} 0,${l.color} 6px,transparent 6px,transparent 9px)` : undefined }} />
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                      <ChartCard
                        ...
                        subtitle={`Average movement speed (px/s)${smoothing ? " — smoothed" : ""}`}
                      >
                        ...
                        <div style={{ display: "flex", gap: 28 }}>
                          {[
                            ...
                            <div key={d.label}>
                              ...
                              <div style={{ fontSize: 20, fontWeight: 600, color: parseFloat(d.val) >= 0 ? "var(--success)" : "var(--err)", fontFamily: "'JetBrains Mono',monospace" }}>
            ...
            {activeTab === "history" && (
              <div className="fade-up">
                ...
                <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 10, overflow: "hidden" }}>
                  <PastJobsTable jobs={pastJobs} onReload={fetchPastJobs} />
                </div>
              </div>
            )}
